...
setwd("/gws/ssde/j25b/ceh_generic/dylcar/indicators_2025/Weevils_Sp")

# Format Weevils data - remove already completed species
data = read.csv("Data_WEE.csv") %>%
rename(date = Start_Date, gridref = Source_Location, species = Taxon_out) %>%
mutate(species = gsub("/", "_", species), species = gsub("=", "", species),
date = ymd(date), year = year(date), gridref)

# Use same name as previos runs/specify name of new run
jobname = "_rslurm_Weevils"
...
saveRDS(visitData, "Weevils_Sp_filtered_start_years.rds")
